  ...
  async function handleSave() {
    ...
    if (personalMode && personalStorageKey) {
      const nextSettings: WorkspaceSettings = {
        ...
        weekdaySettings: form.weekdaySettings.map((weekday) => ({ ...weekday })),
      };

      onSave(nextSettings);
      savePersonalSettings(personalStorageKey, nextSettings);
      setSuccessMessage("내 근무 시간 기준을 저장했습니다.");
      onError(null);
      return;
    }
  ...
  return (
    <>
      {personalMode ? (
        ...
        <div className="rounded-2xl border border-[var(--border)] bg-[var(--surface-2)] px-4 py-3 text-sm text-[var(--text-secondary)]">
          현재는 워크스페이스 기준 근무 시간만 조회할 수 있습니다. 직원별 개인 근무시간 저장 기능은 추가 구조 작업이 필요합니다.
        </div>
      ) : null}
      <div className={`${readOnly ? "pointer-events-none opacity-60 " : ""}space-y-6`}>
        ...
        <section className="settings-card">
          <div className="settings-card__header">
            ...
            <div>
              <h2 className="settings-card__title">{personalMode ? "내 근무 시간" : "근무 시간"}</h2>
          ...
          <div className="settings-card__body space-y-4">
              ...
              {form.weekdaySettings.map((weekday, index) => (
                <div key={weekday.dayIndex} className={`hours-row ${weekday.isOff ? "is-off" : ""}`}>
                  ...
                  <input
                    ...
                    disabled={weekday.isOff || readOnly}
                    onChange={(event) =>
                      setForm((current) => {
            ...
            <div className="flex justify-end gap-2">
              ...
              <button
                ...
                disabled={submitting || readOnly}
              >
                {submitting ? "저장 중..." : personalMode ? "내 근무 시간 저장" : "근무 시간 저장"}
              </button>
            </div>
          </div>
        </section>
